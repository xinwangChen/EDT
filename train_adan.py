...
def adjust_lr(opt, curr_step, total_step=400000, init_lr=5e-4, min_lr=5e-5):#4e-5
    if curr_step%5000==0 and curr_step<=total_step:#s 250000
...
def forward_backward(model, batch, cond, microbatch, schedule_sampler, diffusion, accelerator, mini_step, opt):
    ...
    opt.step()
    return batch_total_loss/mini_step
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for x, y in loader:
            ...
            y = y.squeeze(dim=1)
            cond = dict(y=y)
            batch_loss = forward_backward(model, x, cond, microbatch, schedule_sampler, diffusion, accelerator, mini_step, opt)
            # if train_steps<200000:
            #     clip_grad_norm_(accelerator.unwrap_model(model).parameters(), max_norm=1.0)
            # Log loss values:
            ...
            if train_steps % args.log_every == 0:
                ...
                if accelerator.is_main_process:
                    ...
                    logger.info(f"(step={train_steps:07d}) Avg Loss: {avg_loss:.4f}, Train Steps/Sec: {steps_per_sec:.2f}")
            ...
            if train_steps % 500000 == 0 and train_steps > 0:
                if accelerator.is_main_process:
                    checkpoint = {
