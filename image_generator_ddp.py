"""
Samples a large number of images from a pre-trained EDT model using DDP.
...
def main(args):
    ...
    ckpt_path = args.ckpt or f"EDT-XL-2-{args.image_size}x{args.image_size}.pt"
    ...
    if rank == 0:
        ...
        print(f"Saving .png at {sample_folder_dir}")
    dist.barrier()
    ...
    total_samples = int(math.ceil(args.nnum_images / global_batch_size) * global_batch_size)
    if rank == 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--global-seed", type=int, default=42) # 0, 1
    parser.add_argument("--amm", action=argparse.BooleanOptionalAction, default=False)
